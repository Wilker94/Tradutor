...
def salvar_selecao(voz_atual, config, boost_atual):
    ...
    boost_atual.config(text=f"Boost Atual: {boost}")
    config.destroy()
...
def config_gerais(voz_atual):
    ...
    # Lista de nomes
    nomes = ["James", "Paul", "Harry", "Drew", "Bill", "George", "Adam"]
    ...
    var_combobox = tk.StringVar(value=default_name)
    ...
    btn_salvar = tk.Button(config, text="Salvar", command=lambda: salvar_selecao(voz_atual, config, boost_atual))
